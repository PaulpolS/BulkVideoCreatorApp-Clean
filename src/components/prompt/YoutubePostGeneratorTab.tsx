...
export function YoutubePostGeneratorTab() {
  ...
  useEffect(() => {
    try {
      const it = localStorage.getItem('yt_image_templates');
      if (it) setImageTemplates(JSON.parse(it));
      const at = localStorage.getItem('yt_article_templates');
      if (at) setArticleTemplates(JSON.parse(at));
      setActiveImageTemplateId(localStorage.getItem('yt_active_img_tpl') || '');
      setActiveArticleTemplateId(localStorage.getItem('yt_active_art_tpl') || '');
      setLogoBase64(localStorage.getItem('yt_page_logo') || '');
      setLogoPosition(localStorage.getItem('yt_logo_position') || 'bottom-right');
      setLogoSize(localStorage.getItem('yt_logo_size') || 'small');
      const hp = localStorage.getItem('yt_headline_presets');
      if (hp) setHeadlinePresets(JSON.parse(hp));
      setActiveHeadlinePresetId(localStorage.getItem('yt_active_hl_preset') || '');
      const cp = localStorage.getItem('yt_completed_posts');
  ...
  const analyzeFolderStyles = async () => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (extractedHeadlines.length > 0) {
           ...
           setHeadlinePresets(prev => {
             ...
             localStorage.setItem('yt_headline_presets', JSON.stringify(newPresets));
             return newPresets;
           });
        }
      }
      ...
      if (updatedTemplates.length > 0) {
        ...
        localStorage.setItem('yt_active_img_tpl', updatedTemplates[updatedTemplates.length - 1].id);
      }
  ...
  // Persist helpers
  const saveImageTemplates = (t: ImageTemplate[]) => { setImageTemplates(t); localStorage.setItem('yt_image_templates', JSON.stringify(t)); };
  const saveArticleTemplates = (t: ArticleTemplate[]) => { setArticleTemplates(t); localStorage.setItem('yt_article_templates', JSON.stringify(t)); };
  const saveLogo = (b64: string) => { setLogoBase64(b64); localStorage.setItem('yt_page_logo', b64); };
  const saveLogoPos = (p: string) => { setLogoPosition(p); localStorage.setItem('yt_logo_position', p); };
  const saveLogoSz = (s: string) => { setLogoSize(s); localStorage.setItem('yt_logo_size', s); };
  const saveHeadlinePresets = (p: HeadlinePreset[]) => { setHeadlinePresets(p); localStorage.setItem('yt_headline_presets', JSON.stringify(p)); };
  const addCompletedPost = (p: CompletedPost) => { const next = [p, ...completedPosts]; setCompletedPosts(next); localStorage.setItem('yt_completed_posts', JSON.stringify(next)); };
  ...
  const analyzeImageStyle = async (file: File) => {
    ...
    try {
      ...
      localStorage.setItem('yt_active_img_tpl', tpl.id);
    } catch(e: any) { setErrorMsg(e.message); }
    setIsAnalyzing(false);
  };
  ...
  const analyzeArticleStyle = async (text: string, name: string) => {
    ...
    try {
      ...
      localStorage.setItem('yt_active_art_tpl', tpl.id);
    } catch(e: any) { setErrorMsg(e.message); }
    setIsAnalyzing(false);
  };
  ...
  const exportCSV = () => {
    ...
    a.download = 'yt_viral_posts_' + new Date().toISOString().split('T')[0] + '.csv';
  ...
  const handleGenerate = async () => {
    ...
    if (urls.length === 0) {
      setErrorMsg('กรุณาใส่ลิงก์ YouTube อย่างน้อย 1 ลิงก์');
      return;
    }
    ...
    try {
      ...
      const transcriptForAI = data.transcript && data.transcript.trim().length > 10 
        ? data.transcript.substring(0, 15000) 
        : '(Transcript not available — generate content based on the video title and channel name only.)';
      if (!data.transcript || data.transcript.trim().length < 10) {
        setErrorMsg('⚠️ ดึง Transcript ไม่ได้ — AI จะเขียนจากชื่อคลิปแทน');
      }
  ...
  const regenerateContent = async (type: 'article' | 'headlines' | 'both') => {
    ...
    try {
      ...
      const regenerateTranscript = videoData.transcript && videoData.transcript.trim().length > 10 
  ...
  const processBulkUrls = async () => {
    ...
    for (let i = 0; i < urls.length; i++) {
      const url = urls[i];
      const taskId = `yt_bulk_${Date.now()}_${i}`;
      ...
      (async () => {
        try {
          // 1. Extract info
          globalTaskStore.updateTask(taskId, { progress: 'กำลังดูดคลิปและแคปหน้าจอ...' });
          const exRes = await fetch('/api/youtube-extract', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ url, time: 30 })
          });
          const exData = await exRes.json();
          if (!exData.success) throw new Error(exData.error || 'Failed to extract YouTube info');

          // 2. OpenRouter AI
          globalTaskStore.updateTask(taskId, { progress: 'กำลังให้ AI เขียนบทความและคิดพาดหัว...' });
          const bulkTranscript = exData.transcript && exData.transcript.trim().length > 10 
            ? exData.transcript.substring(0, 15000)
            : '(Transcript not available — generate content based on the video title and channel name only.)';
          const aiResponse = await fetch('https://openrouter.ai/api/v1/chat/completions', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              ...
              messages: [
                { role: 'system', content: getSystemPrompt() },
                { role: 'user', content: `YouTube Title: ${exData.videoTitle}\nChannel Name: ${exData.channelName}\nTranscript: ${bulkTranscript}` }
              ],
              response_format: { type: "json_object" }
            })
          });
          ...
          const targetImageUrl = exData.screenshotUrls?.[0] || exData.channelAvatar;
          if (!targetImageUrl) throw new Error('ไม่พบรูปภาพสำหรับทำต้นฉบับ');

          const safeTitle = (exData.videoTitle || 'youtube_post').replace(/[^a-z0-9]/gi, '_').toLowerCase().substring(0, 30);
          ...
          const newPost: CompletedPost = {
            id: taskId,
            videoTitle: exData.videoTitle || 'Unknown Video',
            channelName: exData.channelName || 'Unknown Channel',
    ...
    setIsProcessing(false);
    setYoutubeUrls(''); // Clear the input after starting tasks
  };
    ...
    <div className="space-y-6">
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] p-6">
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
          ▶️ YouTube to Viral Post <span className="text-sm bg-orange-600 text-white px-2 py-0.5 rounded-full font-medium">ปรับปรุง</span>
        </h2>
        ...
        <div className="flex flex-col gap-4">
          <textarea 
            placeholder="ใส่ลิงก์ YouTube (บรรทัดละ 1 ลิงก์เพื่อรัน Bulk)" 
                ...
                <div className="space-y-2">
                  {imageTemplates.map(t => (
                    <div key={t.id} className={`flex items-center gap-3 p-2 rounded-lg border cursor-pointer transition-all ${activeImageTemplateId === t.id ? 'border-purple-500 bg-purple-900/20' : 'border-[var(--border-color)] hover:border-white/20'}`}
                      onClick={() => { const newId = activeImageTemplateId === t.id ? '' : t.id; setActiveImageTemplateId(newId); localStorage.setItem('yt_active_img_tpl', newId); }}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); saveImageTemplates(imageTemplates.filter(x => x.id !== t.id)); if (activeImageTemplateId === t.id) { setActiveImageTemplateId(''); localStorage.setItem('yt_active_img_tpl', ''); } }} className="text-red-400 hover:text-red-300 text-xs shrink-0">ลบ</button>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div className="space-y-2">
                  {articleTemplates.map(t => (
                    <div key={t.id} className={`flex items-center gap-3 p-2 rounded-lg border cursor-pointer transition-all ${activeArticleTemplateId === t.id ? 'border-blue-500 bg-blue-900/20' : 'border-[var(--border-color)] hover:border-white/20'}`}
                      onClick={() => { const newId = activeArticleTemplateId === t.id ? '' : t.id; setActiveArticleTemplateId(newId); localStorage.setItem('yt_active_art_tpl', newId); }}>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold truncate">{t.name}</p>
                        <p className="text-[10px] text-gray-500 line-clamp-1">{t.analysis.substring(0, 80)}...</p>
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); saveArticleTemplates(articleTemplates.filter(x => x.id !== t.id)); if (activeArticleTemplateId === t.id) { setActiveArticleTemplateId(''); localStorage.setItem('yt_active_art_tpl', ''); } }} className="text-red-400 hover:text-red-300 text-xs shrink-0">ลบ</button>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="space-y-2 mb-3">
                ...
                <button
                  onClick={() => {
                    ...
                    localStorage.setItem('yt_active_hl_preset', preset.id);
                ...
                <div className="space-y-2">
                  {headlinePresets.map(p => (
                    <div key={p.id} className={`flex items-center gap-3 p-2 rounded-lg border cursor-pointer transition-all ${activeHeadlinePresetId === p.id ? 'border-yellow-500 bg-yellow-900/20' : 'border-[var(--border-color)] hover:border-white/20'}`}
                      onClick={() => { const newId = activeHeadlinePresetId === p.id ? '' : p.id; setActiveHeadlinePresetId(newId); localStorage.setItem('yt_active_hl_preset', newId); }}>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold">{p.name}</p>
                        <p className="text-[10px] text-gray-500 line-clamp-1">{p.content.substring(0, 60)}...</p>
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); saveHeadlinePresets(headlinePresets.filter(x => x.id !== p.id)); if (activeHeadlinePresetId === p.id) { setActiveHeadlinePresetId(''); localStorage.setItem('yt_active_hl_preset', ''); } }} className="text-red-400 hover:text-red-300 text-xs shrink-0">ลบ</button>
                    </div>
                  ))}
                </div>
              )}
      ...
      {(isProcessing || videoData || result) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          
          {/* ข้อมูลที่ดึงจาก YouTube */}
          <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] p-6 space-y-4">
            ...
            {isProcessing && !videoData ? (
              <div className="animate-pulse flex flex-col items-center justify-center py-10 space-y-4">
                ...
                <p className="text-[var(--text-secondary)]">กำลังโหลด Transcript และแคปหน้าจอ (รอประมาณ 10-20 วินาที)...</p>
              </div>
            ) : videoData && (
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  {videoData.channelAvatar ? (
                    <img src={videoData.channelAvatar} className="w-16 h-16 rounded-full object-cover border-2 border-white/10" />
                  ) : (
                  ...
                  <div>
                    <h4 className="font-bold text-lg">{videoData.channelName}</h4>
                    <p className="text-sm text-[var(--text-secondary)] line-clamp-2">{videoData.videoTitle}</p>
                  </div>
                </div>

                <div>
                  <p className="text-sm text-[var(--text-secondary)] mb-2">📷 คลิกเลือกรูปที่จะใช้สร้างภาพใหม่ด้วย AI</p>
                  {videoData.screenshotUrls && videoData.screenshotUrls.length > 0 ? (
                    <div className="grid grid-cols-2 gap-2">
                       {videoData.screenshotUrls.map((url, idx) => {
                          ...
                          >
                             <img src={url} className="w-full object-cover aspect-video bg-black" alt={`Screenshot ${idx+1}`} />
                             {isSelected && (
          ...
          <button
            onClick={() => {
               ...
               addCompletedPost({
                 ...
                 canvasFolder: (videoData.videoTitle || 'post').replace(/[^a-z0-9]/gi, '_').substring(0, 30)
               });
            }}
        ...
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] p-6 space-y-4">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { if (confirm('ลบผลลัพธ์ทั้งหมด?')) { setCompletedPosts([]); localStorage.removeItem('yt_completed_posts'); } }} className="px-3 py-2 bg-red-600/30 hover:bg-red-600/50 text-red-400 rounded-lg text-sm">🗑️</button>
            </div>
          </div>
