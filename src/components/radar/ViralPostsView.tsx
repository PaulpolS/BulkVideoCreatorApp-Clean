...
export function ViralPostsView({ pages }: Props) {
  ...
  const [selectedModel, setSelectedModel] = useState<string>(() => localStorage.getItem('radar_selected_model') || 'google/gemini-2.5-flash');
...
ตอบ JSON เท่านั้น: { "results": [ { "id": 1, "viral_score": 8, "evergreen_score": 5 } ] }`;
  ...
  const MODEL_OPTIONS = [
    { id: 'google/gemini-2.5-flash', name: 'Gemini 2.5 Flash (แนะนำ/เร็ว)' },
    { id: 'google/gemini-3-flash-preview', name: 'Gemini 3 Flash Preview (ใหม่)' },
...
ตอบกลับเป็น JSON format เท่านั้น ในรูปแบบ:
{
  ...
  const [captionModel, setCaptionModel] = useState<string>(() => localStorage.getItem('caption_selected_model') || 'google/gemini-2.5-flash');
...
ตอบเป็น JSON เท่านั้น`;
        ...
        const modelsToTry = [captionModel, ...FREE_FALLBACK_MODELS.filter(m => m !== captionModel)];
        ...
        for (const tryModel of modelsToTry) {
          try {
            ...
            if (!res.ok) {
              ...
              // ถ้า credits ไม่พอ → ลอง free model ถัดไป
              if (/insufficient credits|can only afford/i.test(errText) && tryModel !== modelsToTry[modelsToTry.length - 1]) {
                addCaptionLog('warning', `⚠️ ${postLabel} — ${tryModel} (Error: ${errText.substring(0, 100)}) → ลอง free model...`);
                continue;
              }
            ...
            if (jsonMatch) {
              ...
              const modelLabel = tryModel !== captionModel ? ` (fallback: ${tryModel.split('/')[1]})` : '';
              addCaptionLog('success', `✅ ${postLabel} — สร้างแคปชั่นสำเร็จ${modelLabel}`);
              return {
                post,
            ...
            // ถ้าเป็น credits error + ยังมี model ถัดไป → continue
            if (/insufficient credits|can only afford/i.test(err.message || '') && tryModel !== modelsToTry[modelsToTry.length - 1]) {
              addCaptionLog('warning', `⚠️ ${postLabel} — ${tryModel} (Error: ${err.message}) → ลอง free model...`);
              continue;
            }
            ...
            return {
              ...
              comment2: '',
              comment3: '',
              note: `❌ Error: ${err.message || 'API Error'} — ลองเลือก 🆓 Free model แล้วลองใหม่`,
            };
          }
        }
        ...
        return {
          post,
          caption1: '❌ ทุก model ใช้ไม่ได้',
          caption2: '', caption3: '', comment1: '', comment2: '', comment3: '',
          note: '❌ ลอง Generate Key ใหม่ที่ openrouter.ai',
        };
      });
